...
async function fetchModelList(profile: GeneralApiProfile): Promise<string[]> {
...
export default function GeneralApiProfilePanel({ settings, onCommit }: GeneralApiProfilePanelProps) {
  ...
  const handleFetchModels = async () => {
    ...
    try {
      const ids = await fetchModelList(activeProfile)
      if (ids.length === 0) {
  ...
  const checkAllForCurrentProfile = async () => {
    ...
    const results = await Promise.allSettled(
      allKeys.map(async (key) => {
        ...
        const entry = await checkModelHealth(activeProfile, modelId)
